...
for iteration in range(0,6):#number of iteration
	for variab in range(0,18): #go through all variables
		for change in range(0,11):#try different configs
			...
			ypos=[0,0,0,0,0]
			zpos=[0,0,0,0,0]
			with open('/home/scubo/catkin_ws/src/rovio/cfg/rovio_duo.info', 'r') as file:
			...
			with open('/home/scubo/catkin_ws/src/rovio/cfg/rovio_duo.info', 'w') as file:
			    file.writelines( data )
			#start rovio
			print(naming[variab]+ " wird iteriert mit Wert " +repr(changingvalues[variab]))
			for difbags in range(1,5):
				rovionode = pexpect.spawn('roslaunch /home/scubo/catkin_ws/src/rovio/launch/rovio_node_bag_'+repr(difbags)+'.launch',timeout= 180)
				#print("Rovio startet")
